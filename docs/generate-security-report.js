  ...
  sections: [
    // ── COVER PAGE ──
    {
      ...
      children: [
        ...
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 60 }, children: [new TextRun({ text: "Date: 9 April 2026", font: "Arial", size: 22, color: COLORS.muted })] }),
    ...
    // ── MAIN CONTENT ──
    {
      ...
      children: [
          ...
          rows: [
            new TableRow({ children: [
              new TableCell({ borders, width: { size: 2340, type: WidthType.DXA }, shading: { fill: COLORS.criticalBg, type: ShadingType.CLEAR }, margins: { top: 100, bottom: 100, left: 120, right: 120 },
                children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Critical Items", font: "Arial", size: 20, bold: true, color: COLORS.critical })] }), new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "20 total \u2014 18 done", font: "Arial", size: 18, color: COLORS.critical })] })] }),
              new TableCell({ borders, width: { size: 2340, type: WidthType.DXA }, shading: { fill: COLORS.highBg, type: ShadingType.CLEAR }, margins: { top: 100, bottom: 100, left: 120, right: 120 },
                children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "High Items", font: "Arial", size: 20, bold: true, color: COLORS.high })] }), new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "22 total \u2014 21 done", font: "Arial", size: 18, color: COLORS.high })] })] }),
              new TableCell({ borders, width: { size: 2340, type: WidthType.DXA }, shading: { fill: COLORS.implementedBg, type: ShadingType.CLEAR }, margins: { top: 100, bottom: 100, left: 120, right: 120 },
                children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Implemented", font: "Arial", size: 20, bold: true, color: COLORS.implemented })] }), new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "46 of 52", font: "Arial", size: 18, color: COLORS.implemented })] })] }),
              new TableCell({ borders, width: { size: 2340, type: WidthType.DXA }, shading: { fill: "F5F5F5", type: ShadingType.CLEAR }, margins: { top: 100, bottom: 100, left: 120, right: 120 },
                children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Requires Pro Plan", font: "Arial", size: 20, bold: true, color: COLORS.muted })] }), new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "6 need Supabase Pro or paid GitHub", font: "Arial", size: 18, color: COLORS.muted })] })] }),
            ] }),
          ],
        }),
        spacer(200),
        ...
        checkRow("critical", "Enable Supabase Row Level Security (RLS) on every table", "Implemented", "RLS enabled on all tables via Supabase dashboard. Policies configured for SELECT, INSERT, UPDATE, DELETE per user_id.", "Supabase Dashboard > Policies"),
        spacer(60),
        checkRow("critical", "Enforce email verification before data access", "Implemented", "Confirm sign up enabled in Supabase Authentication settings. Users must verify email before accessing data.", "Supabase Dashboard > Authentication > Email Templates"),
        spacer(60),
        ...
        spacer(60),
        checkRow("high", "Short JWT expiry + refresh token rotation", "Implemented", "Refresh token rotation enabled with 10-second reuse interval. JWT expiry at Supabase default (1 hour). Custom session timeouts require Pro plan.", "Supabase Dashboard > Authentication > Sessions"),
        spacer(60),
        checkRow("high", "Rate-limit login attempts", "Implemented", "Supabase built-in rate limiting verified: 30 sign-ups/sign-ins per 5 min per IP, 2 emails/hour, 30 OTP verifications per 5 min.", "Supabase Dashboard > Authentication > Rate Limits"),
        spacer(60),
        checkRow("critical", "Require MFA for admin/staff accounts", "Documented", "Requires Supabase Pro plan. Enable TOTP MFA for all admin accounts when upgrading.", "Supabase Dashboard > Team Settings (Pro plan)"),
        spacer(60),
        ...
        spacer(80),
        checkRow("critical", "Audit every RLS policy with second user account", "Documented", "RLS is enabled on all tables. Cross-user audit test should be run before launch: log in as User B, attempt to read User A records.", "Supabase Dashboard > SQL Editor"),
        spacer(60),
        checkRow("critical", "Encrypt all PII columns at rest (pgcrypto or Vault)", "Documented", "Requires Supabase Pro plan for Vault. Enable column-level encryption on session notes and profile data when upgrading.", "Supabase Dashboard > Vault (Pro plan)"),
        spacer(60),
        checkRow("critical", "Never store session transcripts in plaintext", "Partial", "Session transcripts are cached locally in AsyncStorage (app-sandboxed on iOS/Android; readable on web devtools). Auth tokens are in secure keychain. Authoritative store is Supabase with AES-256 at rest. Privacy policy now accurately describes this posture; the prior base64 'encryption' layer was removed because it misrepresented protection.", "src/hooks/useAppState.tsx + app/privacy.tsx"),
        spacer(60),
        checkRow("high", "Disable direct DB access from client", "Partial", "Non-sensitive CRUD (profile upsert, invite create) remains client-side under RLS. Invite acceptance was moved to an atomic SECURITY DEFINER RPC (public.accept_invite) to close IDOR vectors. Full Edge Function migration is a future hardening step.", "supabase migration: harden_invite_and_couple_policies"),
        spacer(60),
        checkRow("high", "Enable Postgres audit logging (pgaudit)", "Documented", "Requires Supabase Pro plan. Enable pgaudit extension to log all database operations when upgrading.", "Supabase Dashboard > Extensions (Pro plan)"),
        spacer(60),
        checkRow("high", "Automated daily backups with PITR", "Documented", "Requires Supabase Pro plan ($25/mo). Enables point-in-time recovery. Test restoration monthly when upgrading.", "Supabase Dashboard > Backups (Pro plan)"),
        spacer(60),
        checkRow("high", "Restrict Postgres roles", "Documented", "Create a limited role with only required permissions before production launch. App user should not own schema.", "Supabase Dashboard > SQL Editor"),
        spacer(60),
        ...
        spacer(80),
        checkRow("critical", "Enable secret scanning on GitHub repo", "Documented", "Requires GitHub Advanced Security (paid) for private repos. Not available on free plan. Secrets protected via .gitignore and env vars instead.", "GitHub Settings > Code security"),
        spacer(60),
        ...
        spacer(60),
        checkRow("high", "Signed commits and branch protection on main", "Implemented", "Branch ruleset created on main: require PR before merging, block force pushes, restrict deletions. Configured via GitHub Settings > Branches.", "GitHub Settings > Branches > Rulesets"),
        spacer(60),
        ...
        spacer(80),
        checkRow("high", "Supabase network restrictions - IP allowlist", "Documented", "Requires Supabase Pro plan. Restrict dashboard access to known IPs when upgrading.", "Supabase Dashboard > Settings > Network (Pro plan)"),
        spacer(60),
        ...
        // ── NEXT STEPS ──
        heading("Appendix B: Completed Manual Actions"),
        body("The following dashboard items were manually configured on 9 April 2026:"),
        spacer(80),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "\u2705 RLS enabled on all Supabase tables with per-user policies", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "\u2705 Email verification (Confirm sign up) enabled", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "\u2705 Refresh token rotation enabled (10s reuse interval)", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "\u2705 Rate limiting verified (30 sign-ins/5min, 2 emails/hr, 30 OTP/5min)", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "\u2705 Branch protection on main (require PR, block force push, restrict deletions)", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "\u2705 Dependabot alerts, security updates, and private vulnerability reporting enabled", font: "Arial", size: 20 })] }),
        spacer(200),

        heading("Appendix C: Remaining Items (Require Paid Plans)"),
        body("The following items require Supabase Pro ($25/mo) or GitHub Advanced Security and should be enabled before production launch:"),
        spacer(80),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "Enable MFA (TOTP) for admin accounts \u2014 Supabase Pro", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "Enable pgcrypto/Vault for column-level PII encryption \u2014 Supabase Pro", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "Enable pgaudit extension for database audit logging \u2014 Supabase Pro", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "Configure automated backups with PITR \u2014 Supabase Pro", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "Supabase network restrictions (IP allowlist) \u2014 Supabase Pro", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "GitHub secret scanning + push protection \u2014 GitHub Advanced Security", font: "Arial", size: 20 })] }),
        spacer(200),

        heading("Appendix D: Pre-Launch Checklist"),
        body("Additional items to complete before public launch:"),
        spacer(80),
        ...
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "Audit RLS policies with second user account (cross-user test)", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "Restrict Postgres roles (least-privilege)", font: "Arial", size: 20 })] }),
        ...
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "Conduct penetration test (OWASP Top 10)", font: "Arial", size: 20 })] }),
        new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { after: 80 }, children: [new TextRun({ text: "Verify Supabase data residency (South Africa or EU region for POPIA)", font: "Arial", size: 20 })] }),
      ],
    },
  ],
});
